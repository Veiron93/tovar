...
const IndexAuth = function () {
	...
	const handleLogin = (email:any) => {
	  axios.get(`${process.env.REACT_APP_SERVER_LINK}api/users/exists/?email=${encodeURIComponent(email)}`)
	  .then((response) => {
	...
	const handleMailConfirm = (code:any) => {
		...
		axios.post(`${process.env.REACT_APP_SERVER_LINK}api/users/confirm-email/`, data)
		.then((response) => {
			loginUser();
		  })
		.catch((error) => {
	  ...
	  const forms = [
		...
		{
		  key: 2,
		  component: <PasswordForm handleClick={handlePassword} title={isUserExists ? 'Войти' : 'Создать аккаунт'}/>,
